...
新增 QQ 开放平台管理页：支持扫码 / 手动填写 / 复用面板 QQ 登录 / 自建扫码适配器四种开发者登录态来源，统一管理机器人信息、成员、Ark 模板、域名报备、IP 白名单、事件订阅、Webhook、沙箱、可用范围、密钥 Token、官方语料与高级接口。

### 框架升级（框架文件变更）

**框架修复/新增说明**：新增 src/core/open-platform.ts 上游适配器（AES-256-GCM 会话加密、login 多来源模式与字段映射）、src/admin/routes/open-platform.ts 路由、src/admin/web/open-platform.html 独立管理页；面板侧边栏新增入口。

- 新增：feat(开放平台): QQ 开放平台管理页与多来源登录适配器（0d1f1c4）
- 新增：docs: 新增插件开发 AI 提示词与重生成插件索引总览（2408ab8）
...
全插件 @用户 显示真实昵称；群排行同一物理群按真实群号合并并展示各机器人对比；被动回复超限自动降级重发；版本卡动态列出全部插件

### 框架升级（框架文件变更）

**框架修复/新增说明**：collectGroupRanking 按真实群号合并同一群并附各机器人今日消息数；群文本/群Markdown/群图片命中被动回复超限(40034128)时去掉 msg_id 主动重发一次

- 新增：feat: @提及真实昵称、群排行按真实群号合并、被动回复超限降级（3199935）
...
娱乐群管词库支持【显示文字=>指令】外显标记与按群/按指令使用统计；复读插件重写（防回显+限频、代码块/{time}底部）；新增全插件指令/变量/调用目录与变量首字提示；接入 XTCat 修仙世界词库

### 框架升级（框架文件变更）

**框架修复/新增说明**：面板新增指令/变量/调用目录（点击复制或直达词库编辑）与变量首字自动提示；新增 GET /api/plugins/_catalog 与 GET /api/plugins/_dict/stats 接口

- 新增：feat(娱乐群管): 词库外显标记与使用统计、复读插件重写、全插件指令变量目录（9dddfea）
...
新增 DIC管理.php（群内词库列表/测试/校验/切换等）；娱乐群管引擎兼容未闭合 $ 命令并支持 .dic_active 多词库；丰富版词库新增 24 条游戏规则；面板 DIC 入口支持 PHP 插件

### 框架升级（框架文件变更）

**框架修复/新增说明**：菜单编辑器/插件管理放宽 DIC 插件识别，DIC管理.php 也可进入 cid 词库管理；菜单测试新增词库测试按钮

- 新增：feat(词库): 新增 DIC管理.php 与丰富版娱乐群管词库，引擎兼容未闭合 $ 命令并支持 .dic_active 多词库（1533021）
...
绑定QQ后自动补全QQ昵称；OpenID列表与用户管理展示昵称多级兜底

### 框架升级（框架文件变更）

- 修复：fix(昵称同步): 绑定QQ后自动补全QQ昵称，OpenID列表/用户管理展示兜底解析（6b8b698）
...
修复绑定管理：@提及与命令/QQ号紧贴时（绑定QQ 123@用户、解绑QQ@用户）无法识别；现已支持无空格@与@在前/在后两种顺序

### 插件升级 / 新增插件（plugins/ 目录变更）

- 修复：fix(绑定管理): 兼容 @提及与命令/QQ号紧贴的格式（绑定QQ 123@用户、解绑QQ@用户），参数解析先剥离<@...>再取值（725d35a）
...
群人数可编辑并锁定、绑定QQ群支持人数/按群号解绑；成员同步/用户管理/OpenID列表支持多选与批量清空

### 框架升级（框架文件变更）

**框架修复/新增说明**：群人数语义修正（真实人数与已收录分开）、批量删除接口、群内绑定归属对应机器人

- 新增：feat: 群人数可编辑并锁定、绑定QQ群支持人数与按群号解绑、成员/群/OpenID支持多选与批量清空（3dc1e88）
...
修复更新装回旧版本（8091源优先+4.2.x旧线识别）；更新提示按当前机器人显示名称；绑定管理支持@用户绑定/解绑、解绑QQ群；GitHub绑定改为按QQ跨机器人

### 框架升级（框架文件变更）

**框架修复/新增说明**：更新配置桥接按 bot_id 解析机器人名称，修复多机器人更新提示一律显示「空空在php」

- 修复：fix: 更新按新建版本而非最高版本；更新提示按机器人名；绑定支持@用户/解绑群号；GitHub绑定按QQ（839f880）
...
生命周期日志中文化（机器人启动/停止）；开机状态广播改为每个机器人各自向自己所在群发送（含状态图）；复读插件按机器人+群隔离并防止自回显刷屏

### 框架升级（框架文件变更）

**框架修复/新增说明**：机器人启停日志改用中文；bot-status 接口支持按 bot_id 查询；复读状态按机器人隔离

- 修复：fix: 生命周期日志中文化；开机广播按机器人各自群发送；复读按机器人隔离并防自回显刷屏（5bbdad3）
...
修复中文名 PHP 插件临时文件互相覆盖：复读.php/终端.php 不再被彼此覆盖，复读正常响应

### 框架升级（框架文件变更）

- 修复：fix(php-plugin): 中文名插件临时文件互相覆盖导致无反应；全量包纳入站点文件（70fa5bd）
...
复制运行记录级别/类别中文化；仪表盘移除补丁版本卡片；确保全量包包含复读.php 等中文名插件

### 框架升级（框架文件变更）

- 修复：fix(admin): 复制运行记录时级别/类别也中文化；仪表盘移除「补丁版本」卡片（a9f5f32）
- 变更：chore: 忽略 git bundle 增量包（dda632e）
...
修复框架全量包漏掉中文名插件（复读.php 等）；插件全量包排除缓存/临时文件

### 框架升级（框架文件变更）

- 修复：fix(release): 框架全量包禁用 quotepath，避免中文名插件（复读.php 等）被静默漏包；插件全量排除 __pycache__/.tmp（38e5d6f）
...
修复面板每个页面标题重复显示（仪表盘/我的机器人等只保留一个）；桌面显示页面标题、移动端显示顶栏标题

### 框架升级（框架文件变更）

- 修复：fix(admin): 页面标题去重（桌面隐藏顶部标题、移动端隐藏页面 h2，不再出现两个仪表盘）（027f08b）
...
四类更新包分离：框架补丁/插件补丁/框架全量/插件全量各自独立打包与地址；更新系统菜单与面板配置新增四类入口

### 框架升级（框架文件变更）

- 新增：feat(update): 四类更新包分离（框架/插件 × 补丁/全量）、版本号重排脚本、面板与桥接配置（90bde2c）
- 修复：fix(release): 更新日志范围判定排除 zip 产物，避免自动发布提交计入框架升级（0876d6b）
...
补丁版本号重置，改从 1.0.0 起算；框架版本与插件版本分开维护与展示

### 框架升级（框架文件变更）

**框架修复/新增说明**：仪表盘「系统版本」改为显示框架版本，另列补丁版本与插件版本；运行记录级别/类别中文化；系统已运行改为「X天Y小时」；部署终端 PM2 启停/进程表/host metrics 输出中文化；定时任务补充开关关闭/内容为空/图片发送失败的运行记录诊断。

- 新增：feat: 版本体系(框架/插件分离)、运行记录中文、复读插件、定时任务加固、终端中文（baa7732）
- 新增：feat(schedule): 图片发送降级写运行记录，面板恒显发送方式徽章（d9bd548）
- 新增：feat(update): 恢复云端更新配置多源兜底（GitHub raw/Pages/镜像 + 8091）（4fc89af）
- 修复：fix(release): 修正 git log 格式串管道符被 shell 解析（983dd50）
- 修复：fix(admin): 插件列表显示名去除 .php/.py/.js 后缀（d619502）
- 修复：fix(bot): 升级后为已有分配机器人补齐 PHP/PY 分配并修正仪表盘版本显示（aca39e4）
- 修复：fix(bot): 重启服务不再把面板已停止的机器人显示/拉起为开启（9bb8d51）
- 修复：fix(plugin): PHP/PY 插件遵循按机器人分配，未勾选/无记录不再响应（89a2a54）
- 修复：fix(dashboard): 系统版本回退 package.json，机器人计数回退数据库（2af942f）
- 修复：fix(update): 群内更新提醒标注机器人名称；PHP 更新源恢复多源兜底；Markdown 即时发送（ab20288）
- 修复：fix(php-plugin): 插件超时回传已累积回复，修复更新/长任务'没有任何回复'（c3283c5）
- 修复：fix(重启控制): 状态广播外显链接改走 markdown 发送，修复群里显示源码（b1b6936）
- 变更：release: 4.2.100（自动发布）（645259a）
- 变更：chore: 忽略本地运维脚本与完整 dist 包（05ea702）
- 变更：release: 4.2.99（自动发布）（4529e4a）
- 变更：release: 4.2.98（自动发布）（3d2c231）
- 变更：release: 4.2.97（自动发布）（3177b1e）
- 变更：release: 4.2.96（自动发布）（dc0d5fd）
- 变更：release: 4.2.95（自动发布）（3dcf7b8）
- 变更：release: 4.2.94（自动发布）（9f2002e）
- 变更：release: 4.2.93（自动发布）（1869e3b）
- 变更：release: 4.2.92（自动发布）（b968bbd）

### 插件升级 / 新增插件（plugins/ 目录变更）

- 新增：feat: 版本体系(框架/插件分离)、运行记录中文、复读插件、定时任务加固、终端中文（baa7732）
- 修复：fix(php): 无 php-curl 扩展时即时回复/更新下载回退 file_get_contents/copy；下载失败回显具体原因（0bebb4b）
- 修复：fix(update): 群内更新提醒标注机器人名称；PHP 更新源恢复多源兜底；Markdown 即时发送（ab20288）
- 修复：fix(重启控制): 状态广播外显链接改走 markdown 发送，修复群里显示源码（b1b6936）
...
插件列表显示名去除 .php/.py/.js 后缀（类型列已标注类型，名字更清爽）

### 提交

- d619502 fix(admin): 插件列表显示名去除 .php/.py/.js 后缀
- 05ea702 chore: 忽略本地运维脚本与完整 dist 包
- 5412497 chore(site): 自动刷新版本/补丁/插件列表（2026-09-11T07:02Z）
- 4529e4a release: 4.2.99（自动发布）
- aca39e4 fix(bot): 升级后为已有分配机器人补齐 PHP/PY 分配并修正仪表盘版本显示
- be70689 chore(site): 自动刷新版本/补丁/插件列表（2026-09-11T05:32Z）
- 3d2c231 release: 4.2.98（自动发布）
- 9bb8d51 fix(bot): 重启服务不再把面板已停止的机器人显示/拉起为开启
- db1994a chore(site): 自动刷新版本/补丁/插件列表（2026-09-11T05:07Z）
- 3177b1e release: 4.2.97（自动发布）
- 89a2a54 fix(plugin): PHP/PY 插件遵循按机器人分配，未勾选/无记录不再响应
- 96233ff chore(site): 自动刷新版本/补丁/插件列表（2026-09-10T16:03Z）
- dc0d5fd release: 4.2.96（自动发布）
- ac8d40f chore(site): 自动刷新版本/补丁/插件列表（2026-09-10T15:59Z）
- 3dcf7b8 release: 4.2.95（自动发布）
- 0bebb4b fix(php): 无 php-curl 扩展时即时回复/更新下载回退 file_get_contents/copy；下载失败回显具体原因
- e493c36 chore(site): 自动刷新版本/补丁/插件列表（2026-09-10T15:47Z）
- 9f2002e release: 4.2.94（自动发布）
- 2af942f fix(dashboard): 系统版本回退 package.json，机器人计数回退数据库
- ab20288 fix(update): 群内更新提醒标注机器人名称；PHP 更新源恢复多源兜底；Markdown 即时发送
- c3283c5 fix(php-plugin): 插件超时回传已累积回复，修复更新/长任务'没有任何回复'
- c60dde7 chore(site): 自动刷新版本/补丁/插件列表（2026-09-10T14:51Z）
- 1869e3b release: 4.2.93（自动发布）
- d9bd548 feat(schedule): 图片发送降级写运行记录，面板恒显发送方式徽章
- 4fc89af feat(update): 恢复云端更新配置多源兜底（GitHub raw/Pages/镜像 + 8091）
- 0c4328e chore(site): 自动刷新版本/补丁/插件列表（2026-09-10T12:28Z）
- b968bbd release: 4.2.92（自动发布）
- b1b6936 fix(重启控制): 状态广播外显链接改走 markdown 发送，修复群里显示源码
- 4f5813c chore(site): 自动刷新版本/补丁/插件列表（2026-09-10T11:50Z）
...
升级后自动为已有分配记录的机器人补齐 PHP/PY 插件分配；修正仪表盘版本显示
...
插件现在遵循「按机器人分配」：机器人未勾选的插件不再响应，只回已分配的插件；无分配记录时为全局模式。修复重启服务后已停止的机器人又被显示/拉起为开启的问题。
...
修复：PHP/PY 插件现在遵循「按机器人分配」，机器人未勾选的插件不再响应；仅勾选（已分配）的插件回复。无任何分配记录时仍为全局模式（跑全部启用插件）。
...
累计补丁（自 4.2.91）：PHP插件超时回传回复、无curl回退、更新提醒标注机器人名、多源兜底、仪表盘兜底、重启广播外显修复
...
无 php-curl 时回复与更新下载自动回退；下载失败回显原因
...
PHP插件超时回传累积回复；群内更新提醒标注机器人名；PHP更新源多源兜底；仪表盘版本与机器人计数兜底
...
恢复云端更新配置多源兜底；定时任务图片降级写运行记录并恒显发送方式
...
修复重启控制状态广播的菜单外显：含 mqqapi 链接的状态文本改走 sendMarkdownGroup，群里显示为可点外显文字；markdown 失败或无外显权限时自动退回纯文字标签，杜绝显示 markdown 源码；同时修复 statusText 内存行引用未定义 self 的崩溃。新增 2 项冒烟断言。
...
娱乐群管菜单完整化：新增功能大全/指令大全，主菜单与管理菜单补齐入口并支持 url 跳转按钮；引擎新增群消息/单聊消息/发送频道消息/艾特命令，按钮支持 url: 与 at: 语义

### 提交

- aa75d43 feat(娱乐群管): 功能大全与消息命令补全
- 2945843 chore(site): 自动刷新版本/补丁/插件列表（2026-09-10T08:23Z）
...
娱乐群管功能补全：引擎新增批量禁言/撤回/外显/成员解析命令，群管动作失败如实反馈；禁言支持分钟，发积分统一解析目标并即时可见；清屏先撤回再顶屏；菜单按钮+文字外显双入口；默认词库同步到 plugins/词库/

### 提交

- 738c093 feat(娱乐群管): 补全群管动作/积分/清屏/菜单双入口
- 39d5bfd chore(site): 自动刷新版本/补丁/插件列表（2026-09-10T05:04Z）
...
修复娱乐群管词库引擎静默不回复：同名 .txt 文件插件占位导致 娱乐群管.js 从未加载（面板显示已启用但无监听器）；现自动升级占位行为 code 插件并启用。同时修复禁用后仍响应、保存词库重载后旧监听器重复叠加的问题

### 提交

- 5edbde1 fix(plugin): 同名 .txt 占位行不再挡住 .js 引擎加载，修复词库静默不回复
- 4c31fa7 chore(site): 自动刷新版本/补丁/插件列表（2026-09-09T20:53Z）
...
机器人详情与插件管理页的分配表支持 全选/多选 + 批量「已分配/未分配」，按分配运行模式下只有已分配插件响应；批量进入/退出全局模式均有明确提示

### 提交

- 5824919 feat: 机器人插件分配表支持 全选+多选+批量「已分配/未分配」
- abc7680 chore(site): 自动刷新版本/补丁/插件列表（2026-09-09T19:41Z）
...
修复测试.py抢词库指令(娱乐群管/菜单可触发)；词库编辑器新增一键新建随机模板

### 提交

- b7c4099 fix: 测试.py 不再吞掉娱乐前缀/菜单，交给 lzyqzb 词库；词库编辑器新增「新建随机」模板
- e685936 chore(site): 自动刷新版本/补丁/插件列表（2026-09-09T18:43Z）
...
词库编辑器区分执行行+词库迁至plugins/词库/、插件与机器人页支持批量启停

### 提交

- fcd5589 feat: 词库编辑器识别执行行 + 词库统一迁至 plugins/词库/ + 插件/机器人批量启停
- 1d3b659 chore(site): 自动刷新版本/补丁/插件列表（2026-09-09T16:46Z）
...
仪表盘融入运行总览/24小时事件趋势/系统资源/实时事件feed；发送侧撤销内容去重(各机器人消息唯一)，运行记录按机器人标注名称+[appId]；msg_seq 按机器人持久化递增

### 提交

- 09f14fc feat: 仪表盘融入运行总览/24小时事件趋势/系统资源/实时机器人状态与事件feed
- 848de61 fix: 发送侧去掉内容去重；运行记录每行标注机器人ID（名称+appId）
- 0c4d0f1 chore(site): 自动刷新版本/补丁/插件列表（2026-09-09T15:36Z）
...
msg_seq 按机器人持久化递增并补全纯文本序号；发送侧重复抑制；词典跳过按钮卡片与自我介绍回显（修 40054005 去重导致不回复）

### 提交

- f9ff0ad fix: msg_seq 按机器人持久化递增并补全纯文本回复序号；发送侧短窗口重复抑制；词典跳过按钮卡片与自我介绍回显
- 815ba34 chore(site): 自动刷新版本/补丁/插件列表（2026-09-09T15:06Z）
...
娱乐群管命中/发送失败诊断日志；GitHub绑定兜底收窄不再抢答「绑定主人」等其他指令

### 提交

- 1ed592c fix: 娱乐群管加命中/发送失败诊断日志；GitHub绑定兜底收窄不再抢答绑定主人
- 1c95b35 chore(site): 自动刷新版本/补丁/插件列表（2026-09-09T14:32Z）
...
娱乐群管：修复 QQ 群 @机器人(<@!ID>) 前缀不命中致词库零回复；词库去注释精简并补绑定主人别名/空参帮助

### 提交

- 590cb46 fix(lzyqzb): 兼容 <@!ID> 提到前缀、词库去注释精简、补绑定主人别名与空参帮助
- c8f8091 chore(site): 自动刷新版本/补丁/插件列表（2026-09-09T14:09Z）
...
娱乐群管：标准词库头 + 引擎对齐变量大全 v1.3（文本函数族/延时/群管动作回执）+ 全新 34 规则词库（签到/猜大小/今日运势/放烟花/@提及留言等）

### 提交

- c09b1a3 feat(lzyqzb): 按变量大全 v1.3 重生成完整娱乐群管词库并扩充冒烟
- 92cb3fe feat(lzyqzb): 运行器对齐 lzyqzb 变量大全 v1.3 文本函数族与延时回调
- 60bc41e chore(dict): 娱乐群管词库头改为标准 lzyzqzb 格式（词库 娱乐群管 / 版本 1）
- b919019 chore(site): 自动刷新版本/补丁/插件列表（2026-09-09T09:05Z）
...
修复：群消息误走频道接口(11263)；补发娱乐群管引擎与词库(4.2.79 漏包)；词典回复群内回复修复

### 提交

- d90d585 fix(group): 群事件不再伪造 channelId，词典回复群内优先回群消息
- 68ccb75 chore(site): 自动刷新版本/补丁/插件列表（2026-09-08T23:19Z）
- 8570cf1 release: 4.2.79（自动发布）
- 4d7c650 chore(dict): 词库头部补引导注释（确保后续补丁携带覆盖线上污染文件）
- f14180b chore(site): 自动刷新版本/补丁/插件列表（2026-09-08T23:10Z）
...
修复线上 DIC 词库污染（真正携带文件版）：plugins/dict.txt 覆盖为干净版（# 引导注释 + 空空|我是空空爱追剧娱乐机器人，主人QQ511742399！），清除被官方 key|value 解析误拆的 8 个 lzyzqzb 语法行与 280 余注释行；同包固化娱乐群管 lzyzqzb 词库与解释引擎插件（file-娱乐群管 自动注册启用）及词库管理页文件切换。线上升级完成后请重启服务一次并重进词库管理页确认

### 提交

- 4d7c650 chore(dict): 词库头部补引导注释（确保后续补丁携带覆盖线上污染文件）
- f14180b chore(site): 自动刷新版本/补丁/插件列表（2026-09-08T23:10Z）
- b2fef6e release: 4.2.78（自动发布）
- 2f39c4e chore(site): 自动刷新版本/补丁/插件列表（2026-09-08T03:29Z）
- d00e649 release: 4.2.77（自动发布）
- 128f96b feat(娱乐群管): lzyqzb TXT 词库解释引擎上线（20 项冒烟全过）
- c361198 data: 词库更新（娱乐群管词条 289 行，经管理页写入）
- 22c6ca7 build: 重打 4.2.76 更新包（并入词库入口 UI 39416da，覆盖 8091/GitHub 资产）
- 6ddbddf chore(site): 自动刷新版本/补丁/插件列表（2026-09-07T21:44Z）
- 39416da feat(admin): 词库入口补齐 —— index 插件行对 DIC管理/词典回复 在「编辑」旁新增「📖 DIC词库/词典词库」直达；menu-editor 顶栏插件下拉新增「📖 cid 词库管理」快捷按钮（选中对应插件即显）并滚动定位词库区，非词库插件自动隐藏
- 627d0ad chore(site): 自动刷新版本/补丁/插件列表（2026-09-07T21:29Z）
...
修复线上 DIC 词库污染：dict.txt 覆盖为干净占位（空空|我是空空爱追剧娱乐机器人，主人QQ511742399！），清除被官方解析误拆的 8 个 lzyzqzb 语法行与 280 余注释行；同包固化 娱乐群管 lzyqzb TXT 词库与解释引擎插件（file-娱乐群管 自动注册启用）及词库管理页文件切换（dict.txt / 娱乐群管.txt）。线上建议升级完成后重启一次服务并重进词库管理页确认
...
娱乐群管 lzyqzb TXT 词库解释引擎插件上线（规则/触发/回复文本/$读$/$写$/$按钮$/条件/群管动作，20 项冒烟全过）；词库管理页新增文件切换（dict.txt 词典回复 / 娱乐群管.txt lzyqzb 引擎）并按文件名热重载；还原 dict.txt 官方占位
...
- **修复多机器人定时任务群全部停发的根因**：任务仅绑定发送机器人（groups 为空）时，旧逻辑把全库所有机器人名下的群 OpenID 混在一起发给单一机器人 → QQ 平台对不属于该机器人的群 OpenID 返回 11255/群已注销 → 活跃群被误登记不可达而停发（含两台机器人互相把对方的群标成已注销），最终表现为除个别群外全部定时播报停止
- **目标群按机器人隔离**：定时任务绑定机器人后，目标群只取该机器人名下群（group_members/groups 按 bot_id 过滤），不再混入其他机器人视角的群 OpenID
- **不可达判断按机器人维度**：群是否停发以「发送机器人 + 群」组合判断，A 机器人对 B 名下群的误标不再连坐 B 的正常发送
- **自愈清理误停发记录**：每次定时扫描自动清除「登记机器人 ≠ 群实际归属机器人」的脏停发记录（此前盒子上 7 条活跃群全部是这类误标），无需手工清理即可自动恢复

## 2026-09-05

### 4.2.71：重启/启动广播完整状态卡 + 运行记录机器人标识与多选复制
- **重启/启动广播升级完整状态卡**：标注是哪台机器人发出（名称 + ID）、机器人状态、服务器已运行时长、系统总内存/已用/百分比（`/api/bot/php-bridge/bot-status` 扩展 os 信息）、实际加载插件数（引擎真实计数，不再按目录粗算）、平台版本、所在群数，带「✅ 重启完成 · 用时 X 秒」计时
- **外显文字指令菜单**：状态广播底部附加可点击「测试菜单」(`mqqapi://aio/inlinecmd` 回填指令)
- **运行记录增强**：表格新增「机器人」列（按 appId 反查注册表机器人名称）；新增「复制所选」，勾选多条后一键复制，文本含时间/级别/类别/机器人/内容与详情
- **修复**：非超主查看运行记录时机器人过滤字段与存储字段不一致（日志存 appId，原按注册 id 过滤导致漏记录）
