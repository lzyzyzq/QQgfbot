...
插件词库市场支持超主从 plugins 文件夹直接上架插件/词库（审核+定价），市场条目支持完整源码预览

### 框架升级（框架文件变更）

**框架修复/新增说明**：市场导入接口支持类型与定价参数、新增源码全文预览接口

- 新增：feat(market): 超主从 plugins 文件夹上架插件/词库（审核+定价）+ 市场条目完整源码预览（d801ba1）
...
插件词库市场支持超主从 plugins 文件夹直接上架插件/词库（审核+定价），市场条目支持完整源码预览（行号/复制）

### 框架升级（框架文件变更）

**框架修复/新增说明**：市场导入接口支持类型与定价参数、新增源码全文预览接口、市场前端上架与预览 UI
...
新增个人中心（头像/昵称/密码）+ 注册页邮箱验证码注册 + 邮箱绑定体系（本人换绑需验证码、超主免验证改他人）+ SMTP 两级通道与发件显示名可配置

### 框架升级（框架文件变更）

**框架修复/新增说明**：邮件服务模块（nodemailer + 验证码引擎持久化）、注册与邮箱绑定接口、SMTP 配置接口、个人中心接口（头像/昵称/资料）

- 新增：feat(email): 注册页邮箱验证码注册 + 本人换绑邮箱需验证码 + 超主免验证改他人邮箱 + SMTP 配置（e0969dd）
- 新增：feat(email): SMTP 两级配置（超主配置优先，回退系统默认通道 smtp.default.*）+ 未就绪行内提示优化（32ccdfe）
- 新增：feat(email): 发件人显示名可配置 + 验证码持久化（重启不失效）（decd429）
- 新增：feat(profile): 新增个人中心页面（头像上传/修改昵称/修改密码/额度金币注册时间展示）（ca5f556）
...
以1.0.15为基线全量升级：合入市场/主人授权/词库管理 + 超主权限体系（封用户/禁言/封插件）+ 复读问候刷屏根治 + 批量主人群授权 + 手机端增强

### 框架升级（框架文件变更）

**框架修复/新增说明**：含 1.0.16 全部框架变更与 1.0.17 新增：blocklist 黑名单模块、owner-gate 开发者贯通、webhook 封禁拦截与开发者标注、插件封禁锁定与批量授权接口、签到北京时间修复

- 新增：feat(admin/plugins): 超主权限体系补全 + 刷屏根治 + 移动端增强（e45b224）
- 新增：feat(market): 插件词库市场 + 插件管理卡片化/卸载 + 主人与授权系统 + 词库管理页（194eae7）
- 新增：feat(ai): 新增 AI 智能回复侧边栏与用户金币/侧边栏权限系统（2c0bfd7）
- 新增：style(消息工作台): 进一步缩小并居中，新增返回主界面与浏览器返回键支持（53f373d）
- 新增：feat(消息工作台): 富媒体收发、会话头像、乐观发送与群管理跳转（6b726b4）
- 新增：feat(开放平台): 多来源登录态、会话字段注入与聚合代理适配（b61a912）
- 新增：feat(面板): 仪表盘统计图表、机器人额度与实时同步消息工作台（85df9ff）
- 修复：feat(dicts/market): 新建词库弹窗 + 主人授权弹窗改版 + 作者可编辑发布 + 实时同步跳转修复（10cf6a5）
- 修复：fix(plugins): 修复插件列表 esc 报错 + 新建插件弹窗重构 + php 扫描判重（ad21a18）
- 修复：fix(消息工作台): 支持 HttpOnly Cookie 登录态，面板携带 token 打开工作台（c39688a）
- 修复：fix(消息工作台): 真实头像(qqapp)/软键盘/QQ表情/登录态过期处理，SSE 支持 ?token 鉴权（a62d559）
- 变更：style(消息工作台): 界面居中并压缩手机端尺寸，确保 QQ 浏览器输入框完整可见（cffa1be）

### 插件升级 / 新增插件（plugins/ 目录变更）

- 新增：feat(admin/plugins): 超主权限体系补全 + 刷屏根治 + 移动端增强（e45b224）
- 变更：chore(plugins): 同步面板产生的插件审批记录与测试插件、词库运行数据（06f650f）
...
以 1.0.15 为基线的全量升级包：合入 1.0.16 已发布的全部功能（插件词库市场、主人与授权系统、词库管理页等）与 1.0.17 新增的超主权限体系（封用户/禁言/封插件）、刷屏根治、批量主人与群授权、手机端显示增强。

### 新增功能

- 封用户：超主在「用户管理」一键封禁/解封，黑名单内用户所有群聊/私聊消息机器人不再响应（全局黑名单，服务重启不丢失）
- 禁言：用户管理成员行直接禁言/解除禁言（支持自定义时长），与群管理互通
- 封插件：插件管理新增「封禁」，封禁即停用并锁定（无法启用，直到解封），用于插件刷屏等紧急止损
- 批量设置主人 / 批量设置群授权：「插件管理」「词库管理」支持多选后批量应用（与各条目已有配置合并，不覆盖）
- AI 设置教程：写入「AI 智能回复」页，三步启用 + 金币规则 + 常见问题
- 赞助文案支持跳转链接：设置中用 `[文字](https://链接)` 语法即可让指定文字可点击跳转

### 问题修复

- 复读插件刷屏根治：复读输出被平台回推后因底部实时时间戳导致回声比对失配，形成指数级刷屏（曾刷崩面板）；现在记录机器人输出全文精确比对拦截，连续 10 次回声自动关闭本群复读，「停止」同时清空回声残留
- 问候插件刷屏修复：回复内容本身含触发词（如「早上好」）导致机器人自问自答死循环；现记录最近回复拦截自回声，并叠加 3 秒会话冷却
- 欢迎新用户插件：群成员增加事件重复派发时同群同人 10 分钟内只欢迎一次
- 签到跨天修复：签到按北京时间自然日判定，修复北京时间 0-8 点间（UTC 日期未翻转）无法签到位的问题
- 开发者贯通加固：开发者（超级主人）无需先配置主人即可在群聊/私聊直接使用 owner: 指令；系统日志对开发者消息标注 [开发者] 便于确认识别链路

### 移动端

- 手机端全界面显示增强：卡片网格单列防溢出、弹窗底部面板化（safe-area 适配刘海屏/手势条）、批量操作条与按钮触控目标加大、表格长内容换行不截断、横向溢出防护
...
1.0.16 重新打包：合入插件词库市场、插件管理卡片化/卸载、插件主人与授权系统（owner 指令与群授权门禁）、词库管理页。

### 框架升级（框架文件变更）

**框架修复/新增说明**：新增市场/owner-config 接口与 owner-gate 门禁模块；event-bus 消息派发加入插件授权门禁。
...
新增「消息工作台」实时同步页：按机器人查看群聊/私聊会话、实时预览消息并可直接发送；仪表盘升级为圆环图状态分布、近 14 天消息趋势与机器人额度展示。

### 重新打包说明（2026-09-15）

在 1.0.16 基础上再次重新打包，合入插件词库市场、插件管理卡片化、主人与授权系统与词库管理页：

- 新增：侧边栏「插件词库市场」。词库与插件均可上架售卖（免费或金币定价），支持搜索/分类/分页浏览、每日签到领金币、赞助说明；买家安装时扣款并按平台税率给作者分成，作者本人安装免费；词库落盘 plugins/词库/、插件落盘 plugins/，安装后自动热重载并提示到插件管理分配给机器人。
- 新增：24 小时免审核测试期。发布者分享后 24h 内作品仅自己可见可装（带「测试期」标签），到期进入待审核队列，由超级主人审核通过后对所有用户开放。
- 新增：超级主人在市场可编辑任意商品（名称/版本/价格/分类/内容/访问角色权限）、设为系统内置、直接审核通过或驳回、从服务器本地词库/插件文件一键导入上架，并可配置税率、签到金币与赞助文案。
- 新增：插件管理卡片化。所有插件以卡片展示（版本/作者/描述/类型/审核状态/启用状态），原有全部操作按钮（批准/拒绝、启用/禁用、设置、文档、编辑、词库、管理页面、代码、日志、测试）保留进卡片；新增「卸载」按钮（卸载后插件停止运行并从列表移除，重新安装后才能再次使用）与「主人」按钮。
- 新增：插件「主人与授权」系统。每个插件可配置主人列表（QQ 号/OpenID 增删）、群聊授权记录（手动添加支持天数或永久、可删除）与群聊未授权提示文字（留空静默，同群 60 秒仅提示一次）；未配置授权时所有群聊开放，私聊与主人不受限。
- 新增：群内 owner 指令（配置主人列表后生效）：owner:开启 / owner:关闭 / owner:授权 / owner:授权:7 / owner:取消授权 / owner:查询授权；门禁在事件总线层统一拦截，覆盖 JS/PHP/PY/NapCat 全部插件类型，未授权群消息不再进入插件处理。
- 新增：侧边栏「词库管理」页（骨架）：展示 plugins/词库/ 本地词库文件清单并支持一键跳转词库编辑器编辑词条。
- 修复：管理面板 /api/plugins/_dict/files 被 /:name/files 通配路由截获返回「Plugin not found」，已调整注册顺序。

### 框架升级（框架文件变更）

**框架修复/新增说明**：新增市场、owner-config 等管理接口与 owner-gate 门禁模块；event-bus 消息派发加入插件授权门禁。

### 重新打包说明（2026-09-14）

在 1.0.16 基础上重新打包，合入消息工作台增强、AI 智能回复与金币系统：

- 新增：侧边栏「AI 智能回复」页。消息未命中词库指令时由 AI 模型兜底回复，每次成功调用扣 1 金币；支持选择机器人、启用开关、系统格式供应商、系统共享密钥或自有密钥、BaseURL/模型名/随机度/最大 tokens/记忆轮数、人设（System Prompt）与框架预设人设套用、群聊/私聊生效场景、仅 @机器人触发（推荐）或全量回复、Tools JSON（支持 {{tools_json}} 引用）。
- 新增：AI 在线测试（直调模型，不扣金币、不写入对话记忆，需先保存配置）。
- 新增：用户金币系统。机器人所有者持有金币余额（侧边栏实时显示），AI 兜底每次成功回复扣 1 金币，余额不足自动跳过；超级主人可在面板增减任意用户金币并查看流水。
- 新增：超级主人可为每个用户配置侧边栏可用功能（不勾选=按角色默认），非超主登录后导航按配置过滤。
- 新增：超级主人「模型供应商管理」：维护系统格式供应商（名称/格式/默认地址/默认模型/系统共享密钥），内置「柒柒自适应模型 · openai · https://api.18years.ink/openAi/auto · auto」默认项。
- 修复：未绑定 QQ 时玩家真实头像不显示，改用 QQ 官方 qqapp（appId+openid）头像接口，并保留 qlogo 多主机兜底。
- 修复：手机端软键盘弹出遮挡输入框，改用 visualViewport 动态高度并声明 interactive-widget=resizes-content。
- 新增：QQ 表情面板，与 QQ 客户端经典表情一致，选择后插入 /表情名。
- 修复：登录态过期导致页面报错/打不开，401 时自动清理令牌并返回登录页。
- 修复：工作台读取不到令牌（HttpOnly Cookie）时误跳登录页，改为直接以 Cookie 鉴权，并由面板携带 token 打开工作台。
- 优化：工作台整体居中（最大宽度 1120px），手机端界面与输入区更紧凑，软键盘跟随更稳健，确保 QQ 浏览器下消息输入框完整可见。
- 新增：工作台顶栏「← 主界面」可返回管理面板；手机端浏览器返回键可退回会话列表。
- 优化：界面进一步缩小（顶栏/会话/气泡/工具条单行/输入区），最大化聊天可视区域。
- 修复：实时同步显示「未连接」，SSE 支持 ?token= 鉴权并加入断线轮询兜底。

### 框架升级（框架文件变更）

**框架修复/新增说明**：新增 /api/realtime 会话/历史/SSE/发送接口；仪表盘统计扩展用户、词库、消息、趋势与额度；机器人额度超主 5、普通 1；开放平台适配器支持会话字段注入与聚合代理。

- 新增：feat(开放平台): 多来源登录态、会话字段注入与聚合代理适配（b61a912）
- 新增：feat(面板): 仪表盘统计图表、机器人额度与实时同步消息工作台（85df9ff）
- 新增：feat(消息工作台): 富媒体收发、会话头像、乐观发送与群管理跳转（6b726b4）
- 修复：修复消息工作台真实头像/软键盘/表情/登录态/实时连接（本次重新打包）
