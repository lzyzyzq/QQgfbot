...
elseif (前缀($消息, '更新框架补丁') || 前缀($消息, '更新插件补丁') || 前缀($消息, '更新框架全量') || 前缀($消息, '更新插件全量')) $命令命中 = true;
...
// ========== 辅助 ==========
function 更新按钮行() {
  return 外显('更新框架补丁', '更新框架补丁') . '　' . 外显('更新插件补丁', '更新插件补丁') . '　' . 外显('更新框架全量', '更新框架全量') . '　' . 外显('更新插件全量', '更新插件全量') . "\n" .
         外显('更新补丁', '更新补丁') . '　' . 外显('更新全量', '更新全量') . '　' . 外显('检查更新', '检查更新') . "\n" .
...
// 执行更新（框架补丁 / 插件补丁 / 框架全量 / 插件全量 / 兼容合并补丁·全量）
if (前缀($消息, '更新框架补丁') || 前缀($消息, '更新插件补丁') || 前缀($消息, '更新框架全量') || 前缀($消息, '更新插件全量') || 前缀($消息, '更新补丁') || 前缀($消息, '更新全量')) {
  if (前缀($消息, '更新框架补丁')) { $urls = $cfg['frameworkPatchUrls']; $kind = '框架补丁包'; $fname = 'update-framework-patch.zip'; }
  elseif (前缀($消息, '更新插件补丁')) { $urls = $cfg['pluginPatchUrls']; $kind = '插件补丁包'; $fname = 'update-plugin-patch.zip'; }
  elseif (前缀($消息, '更新框架全量')) { $urls = $cfg['frameworkFullUrls']; $kind = '框架全量包'; $fname = 'update-framework-full.zip'; }
  elseif (前缀($消息, '更新插件全量')) { $urls = $cfg['pluginFullUrls']; $kind = '插件全量包'; $fname = 'update-plugin-full.zip'; }
  elseif (前缀($消息, '更新补丁')) { $urls = $cfg['patchUrls']; $kind = '补丁包'; $fname = 'update-patch.zip'; }
  else { $urls = $cfg['fullUrls']; $kind = '全量包'; $fname = 'update-full.zip'; }
