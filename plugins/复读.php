...
// 4) 防「复读输出自回声死循环」：复读时同步记录机器人输出全文，回推消息若与任一历史输出一致则跳过
//    （输出含 {time} 实时时间戳，输入比对必然失配，必须比对输出本身）。
// 5) 自回声熔断：短时间连续多次命中回声判定时自动关闭本群复读并提示一次，确保即使出现未知回声形态也不会刷崩框架。
...
if ($是开始 || $是停止) {
  ...
  __复读_写('复读开关', $群, false);
  写('复读状态', $状态键, false);
  // 清空统计中的回声记录与限频时间戳，防止洪流残留消息继续触发复读
  __复读_写('复读统计', $统计键, array('count' => (function() use ($统计键) { $s = __复读_读('复读统计', $统计键); return is_array($s) && isset($s['count']) ? (int)$s['count'] : 0; })(), 'ts' => array(), 'recent' => array(), 'echo_streak' => 0));
...
// 清理过期记录：输入记录按防回显窗口保留，输出记录保留 120 秒（QQ 回推可能有较大延迟）
$保留 = max(1, $防回显) * 3;
$输出保留 = 120;
$最近 = array_values(array_filter($最近, function($r) use ($现在, $保留, $输出保留) {
  if (!is_array($r)) return false;
  $age = $现在 - (int)($r['t'] ?? 0);
  if (isset($r['o'])) return $age <= $输出保留;
  return $age <= $保留;
}));

// 防回显（三重判定，任一命中即视为机器人自身回声，直接跳过）：
// a) 输出精确匹配：回推消息等于机器人历史复读输出全文（复读输出含 {time} 实时时间戳，
//    输入比对必然失配，必须比对输出本身——这是「停止后仍在刷屏/指数刷屏」的根治点）
// b) 输入精确匹配：相同内容在防回显窗口内再次出现（原有兜底）
// c) 熔断：连续多次命中回声判定 → 自动关闭本群复读并提示一次，确保任何未知回声形态也不会刷崩框架
$回声命中 = false;
foreach ($最近 as $r) {
  if (!is_array($r)) continue;
  if (isset($r['o']) && (string)$r['o'] === $消息) { $回声命中 = true; break; }
  if ((string)($r['c'] ?? '') === $消息 && ($现在 - (int)($r['t'] ?? 0)) <= $防回显) { $回声命中 = true; break; }
}
$连击 = (int)($统计['echo_streak'] ?? 0);
if ($回声命中) {
  $连击++;
  if ($连击 >= 10) {
    __复读_写('复读开关', $状态键, false);
    __复读_写('复读开关', $群, false);
    写('复读状态', $状态键, false);
    __复读_写('复读统计', $统计键, array('count' => $次数, 'ts' => array(), 'recent' => array(), 'echo_streak' => 0));
    echo json_encode(array('replies' => array(回复文本("检测到复读回声循环，已自动关闭本群复读。如需继续请发送「开始」。"))), JSON_UNESCAPED_UNICODE);
    exit(0);
  }
  __复读_写('复读统计', $统计键, array('count' => $次数, 'ts' => $时间戳, 'recent' => $最近, 'echo_streak' => $连击));
  exit(0);
}
$连击 = 0;
...
// 写入统计：输出全文进入回声记录（输出上限 100 条，覆盖回推延迟较大的场景）
$最近[] = array('c' => $消息, 'o' => $正文, 't' => $现在);
if (count($最近) > 100) $最近 = array_slice($最近, -100);
__复读_写('复读统计', $统计键, array('count' => $次数, 'ts' => $时间戳, 'recent' => $最近, 'echo_streak' => 0));
