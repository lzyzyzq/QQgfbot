// 系统工具 v1.2.0 - 在线时间/版本/更新日志/查巡（数据读取网页后端）
module.exports = {
  manifest: {
    id: 'mod-sys-tools',
    name: '系统工具',
    version: '1.2.0',
    description: '在线时间、版本号、更新日志、查巡（版本卡动态读取全部已加载插件版本，自动去重）',
    author: '511742399'
  },
    ...
    handleCommand: async function(ctx, data) {
      try {
        ...
        if (content === '版本') {
          ...
          // 插件版本：优先读接口返回的实际加载清单（与插件文件同步、自动去重同名取最新），
          // 接口缺失时才回退静态兜底，避免手工清单过时漏项。
          var versionMap = {};
          var order = [];
          if (info && Array.isArray(info.plugins) && info.plugins.length > 0) {
            for (var pi = 0; pi < info.plugins.length; pi++) {
              var pl = info.plugins[pi];
              if (!pl || !pl.name) continue;
              if (!Object.prototype.hasOwnProperty.call(versionMap, pl.name)) order.push(pl.name);
              versionMap[pl.name] = pl.version || '—';
            }
          } else {
            var fallbackNames = ['开关机控制', '主菜单', '娱乐中心', '实用工具', '授权系统', '系统工具', '系统设置', 'DIC管理', '群管理工具', '群信息'];
            for (var fi = 0; fi < fallbackNames.length; fi++) {
              order.push(fallbackNames[fi]);
              versionMap[fallbackNames[fi]] = '—';
            }
          }
          versionMsg += '━━━━━━━━━━━━━━\n插件版本（共' + order.length + '个）：\n';
          for (var oi = 0; oi < order.length && oi < 60; oi++) {
            versionMsg += '• ' + order[oi] + ' v' + versionMap[order[oi]] + '\n';
          }
          var botLabel = '';
          try { botLabel = (ctx.engine && ctx.engine.getBotNameById) ? String(ctx.engine.getBotNameById(data.botId) || '') : ''; } catch(e) {}
          if (botLabel) versionMsg += '\n🤖 当前机器人：' + botLabel;
  ...
  onEnable: function(ctx) {
    ctx.logger.info('系统工具 v1.2.0 已加载');
  }
};
